...
class ExpertModel:
    ...
    def __init__(
        ...
        self._trained = False
        self._constant_features: List[int] = []
        self._expected_n_features: int = -1
    ...
    def _fit_scaler(self, X: np.ndarray) -> None:
        ...
        valid_cols = []
        for i in range(X_finite.shape[1]):
            col_data = X_finite[:, i]
            ...
            if iqr > 1e-8:
                ...
                print(f"Warning: Feature {i} has zero IQR ({iqr:.2e}). Dropping from scaler.")
        
        if not valid_cols:
            raise ValueError("All features have zero variance. Cannot fit RobustScaler. Check your input data.")
        
        if len(valid_cols) < X_finite.shape[1]:
            self._constant_features = [i for i in range(X.shape[1]) if i not in valid_cols]
            X_finite = X_finite[:, valid_cols]
        self.scaler.fit(X_finite)
        self._expected_n_features = X.shape[1]

    def _fill_and_scale(self, X_raw: np.ndarray) -> Optional[np.ndarray]:
        if self._expected_n_features != -1 and X_raw.shape[1] != self._expected_n_features:
            raise ValueError(f"Input feature count {X_raw.shape[1]} does not match expected {self._expected_n_features}.")
        
        X_clean = np.nan_to_num(X_raw, nan=0.0, posinf=0.0, neginf=0.0)
        try:
            if self._constant_features:
                mask = np.ones(X_clean.shape[1], dtype=bool)
                mask[self._constant_features] = False
                X_scaled = np.zeros_like(X_clean)
                X_scaled[:, mask] = self.scaler.transform(X_clean[:, mask])
                return X_scaled
            else:
    ...
    def train_ensemble(self, X_tr_raw: np.ndarray, y_tr: np.ndarray, X_val_raw: np.ndarray, y_val: np.ndarray) -> None:
        ...
        X_tr_clean = np.nan_to_num(X_tr_raw, nan=0.0, posinf=0.0, neginf=0.0)
        X_val_clean = np.nan_to_num(X_val_raw, nan=0.0, posinf=0.0, neginf=0.0)
        
        if X_tr_clean.ndim == 1:
            X_tr_clean = X_tr_clean.reshape(-1, 1)
        if X_val_clean.ndim == 1:
            X_val_clean = X_val_clean.reshape(-1, 1)

        self._fit_scaler(X_tr_clean)
        X_tr = self._fill_and_scale(X_tr_clean)
        X_val = self._fill_and_scale(X_val_clean)
        ...
        if self._constant_features:
            print(f"Warning: {len(self._constant_features)} constant features dropped. Using {X_tr.shape[1]} features.")
        ...
        if XGB_AVAILABLE:
            try:
                ...
                if is_classification and self.n_classes == 2:
                    neg = np.sum(y_tr == 0)
                    pos = np.sum(y_tr == 1)
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if self._expected_n_features != -1 and X.shape[1] != self._expected_n_features:
            raise ValueError(f"Input feature count {X.shape[1]} does not match expected {self._expected_n_features}.")

        X_clean = np.nan_to_num(X, nan=0.0, posinf=0.0, neginf=0.0)
        X_scaled = self._fill_and_scale(X_clean)
        if X_scaled is None:
            if self.task_type == 'classification':
                return np.full((len(X), self.n_classes), 1.0 / self.n_classes)
            else:
    ...
    def save(self, path: str) -> None:
        data = {
            ...
            'expected_n_features': self._expected_n_features
        }
    ...
    def load(self, path: str) -> bool:
        ...
        try:
            ...
            self._constant_features = data.get('constant_features', [])
            self._expected_n_features = data.get('expected_n_features', -1)
...
class ExpertEnsemble:
    ...
    GOLD_FEATURES = [
        'smc_score', 'fvg_bull_fill', 'fvg_bear_fill',
        'buying_pressure', 'selling_pressure',
        'vw_delta', 'vol_ratio', 'close_vs_vwap',
        'rsi_14', 'macd_hist_norm', 'bb_width', 'bb_pct',
        'natr', 'close_vs_ema_20',
        'zscore_divergence', 'close_vs_htf',
        'amihud_illiq', 'vol_of_vol', 'perm_entropy',
        'renko_velocity', 'l2_proxy'
    ]
    ...
    def train(
        ...
        if len(df_train) != len(train_embeds):
            raise ValueError(f"df_train length ({len(df_train)}) does not match train_embeds length ({len(train_embeds)}).")
        if len(df_val) != len(val_embeds):
            raise ValueError(f"df_val length ({len(df_val)}) does not match val_embeds length ({len(val_embeds)}).")

        available_cols = [c for c in self.GOLD_FEATURES if c in df_train.columns]
        if len(available_cols) < len(self.GOLD_FEATURES):
            missing = [c for c in self.GOLD_FEATURES if c not in df_train.columns]
            raise ValueError(f"Ensemble missing gold features: {missing}")

        X_train_raw = df_train[available_cols].values.astype(np.float32)
        X_val_raw = df_val[available_cols].values.astype(np.float32)

        X_train_combined = np.concatenate([X_train_raw, train_embeds], axis=1)
        X_val_combined = np.concatenate([X_val_raw, val_embeds], axis=1)

        for task_name in self.TASK_CONFIGS.keys():
            if task_name in y_train_dict:
                if len(y_train_dict[task_name]) != len(X_train_combined):
                    raise ValueError(f"Target '{task_name}' length {len(y_train_dict[task_name])} does not match features length {len(X_train_combined)}.")
            if task_name in y_val_dict:
                if len(y_val_dict[task_name]) != len(X_val_combined):
                    raise ValueError(f"Target '{task_name}' val length {len(y_val_dict[task_name])} does not match features length {len(X_val_combined)}.")
        ...
        for task_name, model in self.models.items():
            ...
            X_tr_clean = X_train_combined[valid_tr]
            y_tr_clean = y_tr[valid_tr]
            X_val_clean = X_val_combined[valid_val]
            y_val_clean = y_val[valid_val]
            
            if len(X_tr_clean) < 100 or len(X_val_clean) == 0:
                print(f"  Insufficient samples for {task_name}. Skipping.")
                continue
            ...
            if model.is_trained:
                ...
                if model.xgb_model is not None:
                    try:
                        ...
                        pass
                if imp is None and model.lgb_model is not None:
                    try:
