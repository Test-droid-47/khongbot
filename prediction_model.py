...
class PredictionModel:
    ...
    def _engineer_auxiliary_targets(df: pd.DataFrame, horizons: List[int], envelope_horizon: int) -> Dict[str, np.ndarray]:
        ...
        max_down = np.full(n, np.nan, dtype=np.float32)
    ...
    def prepare_data(self, df: pd.DataFrame, feature_cols: List[str] = None, is_training: bool = True):
        ...
        for col in smc_price_cols:
            ...
            numeric_df[col] = np.clip(numeric_df[col], -10, 10)
        ...
        if feature_cols is None:
            feature_cols = self.cfg.get('lstm_features', None)
            if feature_cols is None:
                feature_cols = [
                    'smc_score',
                    'fvg_bull_fill',
            ...
            original_len = len(feature_cols)
            feature_cols = [c for c in feature_cols if c in numeric_df.columns]
            if len(feature_cols) < original_len:
                print(f"Warning: {original_len - len(feature_cols)} LSTM feature(s) missing. Proceeding with {len(feature_cols)} features.")
        else:
            original_len = len(feature_cols)
            filtered = [c for c in feature_cols if c in numeric_df.columns]
            if len(filtered) < original_len:
                missing = [c for c in feature_cols if c not in numeric_df.columns]
                raise ValueError(f"User-provided LSTM features missing from DataFrame: {missing}")
            feature_cols = filtered
            new_regime_cols = [c for c in numeric_df.columns if c.startswith('regime_')]
            feature_cols = feature_cols + new_regime_cols
        ...
        self._feature_cols = [c for c in self._feature_cols if c in all_cat_cols or c in valid_cont_cols]
        ...
        if is_training:
            if self.scaler is None:
                ...
                for col in all_cont_cols:
                    ...
                    iqr = q75 - q25
                    if iqr > 1e-8:
                        valid_cont_cols.append(col)
                    else:
                        print(f"Warning: Feature '{col}' has near-zero variance (IQR={iqr:.2e}). Dropping from scaler to avoid NaN.")
                if len(valid_cont_cols) < len(all_cont_cols):
                    self._feature_cols = [c for c in self._feature_cols if c in valid_cont_cols]
                    all_cont_cols = valid_cont_cols
        ...
        trim_limit = min_len - max(self.horizons)
        if trim_limit <= 0:
            raise ValueError(f"Not enough data after trimming max horizon ({max(self.horizons)}). Need more than {max(self.horizons)} valid samples.")
        
        X = X[:trim_limit]
        y_returns = y_returns[:trim_limit]
        y_envelope = y_envelope[:trim_limit]
        ...
        return X, y_dict, self._feature_cols, -1
    ...
    def load(self, path=None):
        ...
        self._close_idx = c.get('close_idx', -1)
        ...
        print(f"Feature Extractor loaded from {load_path}")
