...
class TradingEnvironment:
    ...
    def _get_expert_signals(self) -> Dict[str, float]:
        ...
        if hasattr(self.df, 'expert_direction_prob') and idx < len(self.df):
            return {
                'direction_prob': float(self.df.iloc[idx]['expert_direction_prob']),
                'expected_move': float(self.df.iloc[idx]['expert_expected_move']),
                'entry_quality': float(self.df.iloc[idx]['expert_entry_quality']),
                'exit_distance': float(self.df.iloc[idx]['expert_exit_distance'])
            }
        return {
            'direction_prob': 0.5,
            'expected_move': 0.003,
    ...
    def _get_ppo_state(self) -> np.ndarray:
        ...
        idx = min(self.current_idx, self.n_bars - 1)
        vol_agg = float(self.df.iloc[idx]['vol_aggression']) if 'vol_aggression' in self.df else 0.0
        range_pct = float(self.df.iloc[idx]['range_percentile']) if 'range_percentile' in self.df else 0.5
    ...
    def _calculate_reward(self, pnl_pct: float, trade_closed: bool = False) -> float:
        ...
        if trade_closed and pnl_pct != 0:
            reward = pnl_pct * 10.0  
            reward += 0.05 * min(self.consecutive_wins, 5) if pnl_pct > 0 else -0.05 * min(self.consecutive_losses, 5)
        
        val = self.get_portfolio_value()
        if val > self.peak_capital: self.peak_capital = val
        drawdown = (self.peak_capital - val) / (self.peak_capital + 1e-10)
        if drawdown > 0.05: reward -= self.drawdown_penalty * drawdown
        if self.capital < 0: reward -= 0.5
        return float(np.clip(reward, -1.0, 1.0))

    def _current_price(self) -> float: return float(self.df['close'].iloc[min(self.current_idx, self.n_bars - 1)])
    def _current_high(self) -> float: return float(self.df['high'].iloc[min(self.current_idx, self.n_bars - 1)])
    def _current_low(self) -> float: return float(self.df['low'].iloc[min(self.current_idx, self.n_bars - 1)])

    def get_portfolio_value(self) -> float:
        price = self._current_price()
        if self.position == 0: return self.capital
        if self.trading_mode == 'spot': return self.capital + (self.position * price)
    ...
    def _force_close_position(self, exit_price: float, log_type: str) -> float:
        ...
        self.position = 0.0; self.entry_price = 0.0; self.margin_locked = 0.0
        self.trailing_activated = False; self.trailing_peak = 0.0
        return trade_pnl

    def _execute_order(self, action: int) -> Tuple[bool, float, str]:
        price = self._current_price()
        if action == 0: return True, 0.0, 'hold'
        ...
        if self.position == 0:
            if action_type == 'close_all': return True, 0.0, 'hold'
            ...
            if action_type == 'buy':
                ...
                if self.trading_mode == 'future':
                    ...
                    if margin + fee <= self.capital:
                        self.capital -= (margin + fee); self.margin_locked = margin
                        self.position = size; self.entry_price = entry_price
                        self.entry_idx = self.current_idx; self.position_side = 'long'
                        self.dynamic_sl = entry_price * (1 - sl_pct); self.dynamic_tp = entry_price * (1 + tp_pct)
                        self.trailing_peak = entry_price
                        return True, size, 'buy_long_open'
                else:
                    cost = trade_value * (1 + self.fee_rate)
                    if cost <= self.capital:
                        self.capital -= cost; self.position = size; self.entry_price = entry_price
                        self.entry_idx = self.current_idx; self.position_side = 'long'
                        self.dynamic_sl = entry_price * (1 - sl_pct); self.dynamic_tp = entry_price * (1 + tp_pct)
                ...
                if margin + fee <= self.capital:
                    self.capital -= (margin + fee); self.margin_locked = margin
                    self.position = size; self.entry_price = entry_price
                    self.entry_idx = self.current_idx; self.position_side = 'short'
                    self.dynamic_sl = entry_price * (1 + sl_pct); self.dynamic_tp = entry_price * (1 - tp_pct)
                    self.trailing_peak = entry_price
        ...
        if self.position > 0 and self.position_side == 'long':
            if action_type == 'buy': return False, 0.0, 'invalid_scaling'  
            if action_type == 'sell' or action_type == 'close_all':
                ...
                if self.position < 1e-10:
                    ...
                    if trade_pnl > 0: self.consecutive_wins += 1; self.consecutive_losses = 0
        ...
        if self.position > 0 and self.position_side == 'short':
            if action_type == 'sell': return False, 0.0, 'invalid_scaling'  
    ...
    def _get_dynamic_sl_tp_pct(self) -> Tuple[float, float]:
        return self.cfg.get('stop_loss_pct', 0.02), self.cfg.get('take_profit_pct', 0.03)
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool]:
        ...
        if self.position > 0 and self.use_trailing:
            price = self._current_price()
            if self.position_side == 'long':
                if price > self.trailing_peak: self.trailing_peak = price
                profit_pct = (price - self.entry_price) / self.entry_price
                if profit_pct >= self.trailing_activation_pct: self.trailing_activated = True
                if self.trailing_activated:
                    new_sl = self.trailing_peak * (1 - self.trailing_callback_pct)
                    if new_sl > self.dynamic_sl: self.dynamic_sl = new_sl
            elif self.position_side == 'short':
                if price < self.trailing_peak or self.trailing_peak == 0.0: self.trailing_peak = price
                profit_pct = (self.entry_price - price) / self.entry_price
                if profit_pct >= self.trailing_activation_pct: self.trailing_activated = True
                if self.trailing_activated:
                    new_sl = self.trailing_peak * (1 + self.trailing_callback_pct)
                    if new_sl < self.dynamic_sl or self.dynamic_sl == 0.0: self.dynamic_sl = new_sl
        ...
        if self.position > 0:
            ...
            if self.position_side == 'long':
                if low <= self.dynamic_sl:
                    stopped_out = True; exit_price = self.dynamic_sl * (1 - self.slippage); log_msg = 'sl_triggered'
                elif high >= self.dynamic_tp:
                    stopped_out = True; exit_price = self.dynamic_tp * (1 + self.slippage); log_msg = 'tp_triggered'
            elif self.position_side == 'short':
    ...
    def get_trade_statistics(self) -> Dict[str, Any]:
        if not self.trades: return {'total_trades': 0, 'win_rate': 0.0, 'avg_win': 0.0, 'avg_loss': 0.0, 'profit_factor': 0.0, 'total_return': 0.0, 'invalid_scaling_attempts': self.invalid_scaling_count}
        ...
        gross_profit = sum(wins); gross_loss = sum(np.abs(losses))
        profit_factor = gross_profit / gross_loss if gross_loss > 0 else 0.0
        return {'total_trades': total_trades, 'win_rate': win_rate, 'avg_win': avg_win, 'avg_loss': avg_loss, 'profit_factor': profit_factor, 'total_return': (self.get_portfolio_value() - self.initial_capital) / self.initial_capital, 'invalid_scaling_attempts': self.invalid_scaling_count}
